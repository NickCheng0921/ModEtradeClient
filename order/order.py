...
class Order:
    ...
    def place_market_order_AUTO(self, ticker, shares):
        ...
        url_prod = self.base_url + "/v1/accounts/" + self.account["accountIdKey"] + "/orders/place.json"

        # Add parameters and header information
        headers = {"Content-Type": "application/xml", "consumerKey": config["DEFAULT"]["CONSUMER_KEY"]}

        # we need to build a preview order first, before we can place an actual one
        ...
        if response is not None and response.status_code == 200:
            previewId = response.json()['PreviewOrderResponse']['PreviewIds'][0]['previewId']
            print(f"    Generated preview: {previewId}")
        else:
            data = response.json()
            if 'Error' in data and 'message' in data["Error"] and data["Error"]["message"] is not None:
                print("Error: " + data["Error"]["message"] + ' ' + str(response.status_code))
            else:
                print("Error: Preview Order API service error")

        """
        BEGIN EXTREME DEGENERACY
        """
        ...
        payload = payload.format(order["client_order_id"], str(previewId), order["price_type"], order["order_term"],
                                 order["limit_price"], order["symbol"], order["order_action"], order["quantity"])
        ...
        if response is not None and response.status_code == 200:
            print(f"    Placed Order: {client_order_id}")
            return response
        else:
            data = response.json()
            if 'Error' in data and 'message' in data["Error"] and data["Error"]["message"] is not None:
    ...
    def place_sell_limit_order_AUTO(self, ticker, shares, stop_loss, isBelow):
        ...
        if response is not None and response.status_code == 200:
            previewId = response.json()['PreviewOrderResponse']['PreviewIds'][0]['previewId']
            print(f"    Preview Sell Limit Order: {previewId} CID: {client_order_id}")
        else:
            data = response.json()
            if 'Error' in data and 'message' in data["Error"] and data["Error"]["message"] is not None:
                print("Error: " + data["Error"]["message"] + ' ' + str(response.status_code))
        ...
        place_order_cid = random.randint(1000000000, 9999999999)
        print(f"    Generated place order CID {place_order_cid}")
        payload = payload.format(place_order_cid, previewId, order["price_type"], order["order_term"],
                                 order["limit_price"], order["symbol"], order["order_action"], order["quantity"])
    ...
    def place_EXTREME_SHIT_order_AUTO(self, ticker, shares):
        ...
        # User's order selection
        order, client_order_id = self.build_order_EXTREME_SHIT_AUTO(ticker, shares)
        ...
        if response is not None and response.status_code == 200:
            previewId = response.json()['PreviewOrderResponse']['PreviewIds'][0]['previewId']
            print(f"    Generated preview: {previewId} CID: {client_order_id}")
        else:
            data = response.json()
            if 'Error' in data and 'message' in data["Error"] and data["Error"]["message"] is not None:
                print("Error: " + data["Error"]["message"] + ' ' + str(response.status_code))
    ...
    def build_sell_limit_order_AUTO(self, ticker, shares, stop_loss):
        ...
        return order, order["client_order_id"]
    
    def build_order_EXTREME_SHIT_AUTO(self, ticker, shares):
        """
            Provides users options to select to preview orders
            :param self test
            :return user's order selections
    ...
    def build_market_order_AUTO(self, ticker, shares):
        ...
        """
        client_order_id
        Reference number generated by developer.
